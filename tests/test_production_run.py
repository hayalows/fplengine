...
class ProductionRunTests(unittest.TestCase):
    def test_run_once_never_initializes_schema(self) -> None:
        ...
        with (
            ...
            self.assertEqual(result["season_events"], "ok")
    ...
    def test_run_once_survives_missing_metadata_table(self) -> None:
        """Optional deadline metadata must never break core ingestion."""
        snapshot = MagicMock()
        snapshot.source_hash = "b" * 64
        snapshot.fetched_at.isoformat.return_value = "2026-08-24T00:00:00+00:00"
        snapshot.bootstrap = {"elements": [1], "teams": [1]}
        snapshot.fixtures = []
        prediction = MagicMock()
        prediction.target_event = 2
        prediction.model_version = "xp-v0.2.0"
        ...
        with (
            ...
            store = store_cls.return_value
            store.is_postgres = True
            store.save_snapshot.return_value = (8, True)
            store.save_predictions.return_value = 12
            store.save_season_events.side_effect = RuntimeError(
                'relation "engine.season_event" does not exist'
            )

            result = run_once("postgresql://example.invalid/neondb")

            store.save_predictions.assert_called_once()
            self.assertEqual(result["prediction_run_id"], 12)
            self.assertIn("skipped", result["season_events"])
